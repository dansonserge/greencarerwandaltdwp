...
function themesflat_shortcode_default_id(){
    return array(
                ...
                'group' => esc_html__( 'Design Options', 'graingrow' ),
...
function themesflat_predefined_header_styles() {
    ...
    if ( empty( $styles ) ) {
        $styles = apply_filters( 'themesflat/header_styles', array(
            'header-v1' => esc_html__( 'Classic', 'graingrow' ),
            'header-v2' => esc_html__( 'Header style 02', 'graingrow' ),
            'header-v4' => esc_html__( 'Modern', 'graingrow' ),
        ) );
    }
...
function themesflat_available_social_icons() {
    $icons = apply_filters( 'themesflat_available_icons', array(
        'twitter'        => array( 'iclass' => 'icon-graingrow-twitter', 'title' => 'Twitter','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),
        'twitter-2'        => array( 'iclass' => 'icon-graingrow-icon10', 'title' => 'Twitter','share_link' => THEMESFLAT_PROTOCOL . '://twitter.com/intent/tweet?url=' ),
        'facebook'       => array( 'iclass' => 'icon-graingrow-facebook', 'title' => 'Facebook','share_link'=> THEMESFLAT_PROTOCOL . '://www.facebook.com/sharer/sharer.php?u=' ),
        'facebook-2'       => array( 'iclass' => 'icon-graingrow-facebook2', 'title' => 'Facebook','share_link'=> THEMESFLAT_PROTOCOL . '://www.facebook.com/sharer/sharer.php?u=' ),
        'google-plus-g'    => array( 'iclass' => 'icon-graingrow-google-plus', 'title' => 'Google Plus','share_link'=> THEMESFLAT_PROTOCOL . '://plus.google.com/share?url=' ),
        'pinterest'      => array( 'iclass' => 'icon-graingrow-pinterest', 'title' => 'Pinterest','share_link' => THEMESFLAT_PROTOCOL . '://pinterest.com/pin/create/bookmarklet/?url=' ),
        'instagram'      => array( 'iclass' => 'icon-graingrow-instagram', 'title' => 'Instagram','share_link' => 'https://www.instagram.com/?url=' ),
        'instagram-2'      => array( 'iclass' => 'icon-graingrow-icon11', 'title' => 'Instagram','share_link' => 'https://www.instagram.com/?url=' ),
        'youtube'        => array( 'iclass' => 'icon-graingrow-youtube', 'title' => 'Youtube','share_link' =>'' ),
        'vimeo'          => array( 'iclass' => 'icon-graingrow-vimeo', 'title' => 'Vimeo','share_link' =>'' ),
        'behance'        => array( 'iclass' => 'icon-graingrow-behance', 'title' => 'Behance','share_link' =>'' ),
        'bitcoin'        => array( 'iclass' => 'icon-graingrow-bitcoin', 'title' => 'Bitcoin','share_link' =>'' ),
        'digg'           => array( 'iclass' => 'icon-graingrow-digg', 'title' => 'Digg','share_link' =>'http://digg.com/submit?url=' ),
        'skype'          => array( 'iclass' => 'icon-graingrow-skype', 'title' => 'Skype','share_link' => THEMESFLAT_PROTOCOL . '://web.skype.com/share?url='),
        'slack'          => array( 'iclass' => 'icon-graingrow-slack', 'title' => 'Slack','share_link' => ''),
        'spotify'        => array( 'iclass' => 'icon-graingrow-spotify', 'title' => 'Spotify','share_link' => ''),
        'stack-overflow' => array( 'iclass' => 'icon-graingrow-stack-overflow', 'title' => 'Stach Overflow','share_link' => ''),
        'steam'          => array( 'iclass' => 'icon-graingrow-steam', 'title' => 'Steam','share_link' => ''),
        'dribble'          => array( 'iclass' => 'icon-graingrow-dribble', 'title' => 'Dribble','share_link' => ''),
        'linkedin-2'          => array( 'iclass' => 'icon-graingrow-icon12', 'title' => 'Steam','share_link' => ''),
        'linkedin2'          => array( 'iclass' => 'icon-graingrow-linkedin2', 'title' => 'Linkedin','share_link' => ''),
    ) );
...
function themesflat_menu_fallback() {
    ...
    <li>
    <a class="menu-fallback" href="' . esc_url(admin_url('nav-menus.php')) . '">' . esc_html__( 'Create a Menu', 'graingrow' ) . '</a></li></ul>';
}
...
if ( version_compare( $GLOBALS['wp_version'], '4.1', '<' ) ) :
    ...
    function themesflat_wp_title( $title, $sep ) {
        ...
        if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
            $title .= " $sep " . sprintf( esc_html__( 'Page %s', 'graingrow' ), max( $paged, $page ) );
        }
            ...
            <ul class="unstyled">
                <li class="current">                    
                    <a href="?lang=en" class="lang_sel_sel">
                        <span class="languages-before-icon icon-graingrow-web"></span><?php echo esc_html__("English",'graingrow');?><i class="icon-graingrow-arrow-down" aria-hidden="true"></i>
                    </a>
                    <ul class="unstyled-child">
                       <li class="icl-en">
                            <a href="?lang=en" class="lang_sel_sel">
                             <?php echo esc_html__("English",'graingrow');?>
                            </a>
                       </li>
                       <li class="icl-fr fr">
                            <a href="?lang=fr" class="lang_sel_other">
                             <?php echo esc_html__("French",'graingrow');?>
                            </a>
                       </li>
                       <li class="icl-ge ge">
                            <a href="?lang=it" class="lang_sel_other">
                                <?php echo esc_html__("German",'graingrow');?>
                            </a>
                       </li>
                    </ul>
                </li>
...
function themesflat_custom_search_form( $form ) {
    ...
    <label>
        <span class="screen-reader-text">' . esc_html__( 'Search for:' , 'graingrow' ) . '</span>
        <input type="search" value="' . get_search_query() . '" name="s" class="s" placeholder="' . esc_html__( "Search…", "graingrow" ) . '"/>
    </label>
    <button type="submit" class="search-submit"><i class="icon-graingrow-seo10"></i></button>    
    </form>';
...
function themesflat_social_single() {
    if( themesflat_get_opt('show_social_share') == 1 ):
        ...
        ?>
        <div class="social-share-article"><h5><?php echo esc_html__( 'Share News', 'graingrow' ); ?></h5>        
...
function themesflat_get_page_titles() {
    ...
    if ( ! is_archive() ) {       
        if ( is_home() ) {
            ...
            if ( is_front_page() ) {
                $title = esc_html__('Blog', 'graingrow');              
            }
        } 
        elseif ( is_page() ) {
            ...
            $title = esc_html__( '404', 'graingrow' );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'graingrow' ), get_search_query() );
        } else {
            $title = get_post_meta( get_the_ID(), 'custom_title', true );
            if ( ! $title ) {
                $title = get_the_title();
        ...
        if ( is_author() ) {
            the_post();
            $title = sprintf( esc_html__( 'Author: %s', 'graingrow' ), get_the_author() );
            rewind_posts();
        } elseif ( is_day() ) {
            $title = sprintf( esc_html__( 'Daily: %s', 'graingrow' ), get_the_date() );
        } elseif ( is_month() ) {
            $title = sprintf( esc_html__( 'Monthly: %s', 'graingrow' ), get_the_date( 'F Y' ) );
        } elseif ( is_year() ) {
            $title = sprintf( esc_html__( 'Yearly: %s', 'graingrow' ), get_the_date( 'Y' ) );
        } elseif ( is_search() ) {
            $title = sprintf( esc_html__( 'Search results for &#8220;%s&#8221;', 'graingrow' ), get_search_query() );
            ...
            $title = esc_html__('Book Appointment','graingrow');           
