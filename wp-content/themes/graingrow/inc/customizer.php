...
function themesflat_customize_register( $wp_customize ) {
   ...
   //___FOOTER___//
    ...
    //___LAYOUT___//
    $wp_customize->get_section( 'background_image' )->title = esc_html__('Layout Style', 'graingrow');
...
function themesflat_sanitize_bg_size( $input ) {
    $valid = array(
        'cover'     => esc_html__('Cover', 'graingrow'),
        'contain'   => esc_html__('Contain', 'graingrow'),
    );
...
function themesflat_sanitize_blog( $input ) {
    $valid = array(
        ...
        'fullwidth'  => esc_html__( 'Full width (no sidebar)', 'graingrow' )
...
function themesflat_sanitize_pagination ( $input ) {
    $valid = array(
        'pager' => esc_html__('Pager', 'graingrow'),
        'numeric' => esc_html__('Numeric', 'graingrow'),
        'page_numeric' => esc_html__('Pager & Numeric', 'graingrow')                
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_post ( $input ) {
    $valid = array(
        'simple_list' => esc_html__('Simple List', 'graingrow'),
        'grid' => esc_html__('Grid', 'graingrow')        
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_fw( $input ) {
    $valid = array(
        '0' => esc_html__('footer_default', 'graingrow'),
        '1' => esc_html__('One', 'graingrow'),
        '2' => esc_html__('Two', 'graingrow'),
        '3' => esc_html__('Three', 'graingrow'),
        '4' => esc_html__('Four', 'graingrow')
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_related_portfolio( $input ) {
    $valid = array(
        'grid'                 => esc_html__( 'Grid', 'graingrow' ),
        'grid_masonry'         => esc_html__( 'Grid Masonry', 'graingrow' ),
        'grid_nomargin'        => esc_html__( 'Grid Masonry No Margin', 'graingrow' ),
        'carosuel'             => esc_html__( 'Carosuel', 'graingrow' ),
        'carosuel_nomargin'    => esc_html__( 'Carosuel No Margin', 'graingrow' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_pagination( $input ) {
    $valid = array(
        'page_numeric'         => esc_html__( 'Pager & Numeric', 'graingrow' ),
        'load_more'         => esc_html__( 'Load More', 'graingrow' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order( $input ) {
    $valid = array(
        ...
        'id'            => esc_html__( 'Id', 'graingrow' ),
        'author'        => esc_html__( 'Author', 'graingrow' ),
        'title'         => esc_html__( 'Title', 'graingrow' ),
        'modified'      => esc_html__( 'Modified', 'graingrow' ),
        'comment_count' => esc_html__( 'Comment Count', 'graingrow' ),
        'menu_order'    => esc_html__( 'Menu Order', 'graingrow' )     
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_portfolio_order_direction( $input ) {
    $valid = array(
        'DESC' => esc_html__( 'Descending', 'graingrow' ),
        'ASC'  => esc_html__( 'Assending', 'graingrow' )       
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio( $input ) {
    $valid = array(
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'graingrow' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'graingrow' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'graingrow' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'graingrow' )
    );
    if ( array_key_exists( $input, $valid ) ) {
        return $input;
    } else {
...
function themesflat_sanitize_grid_portfolio_related( $input ) {
    $valid = array(
        'portfolio-one-columns'     => esc_html__( '1 Columns', 'graingrow' ),
        'portfolio-two-columns'     => esc_html__( '2 Columns', 'graingrow' ),
        'portfolio-three-columns'     => esc_html__( '3 Columns', 'graingrow' ),
        'portfolio-four-columns'     => esc_html__( '4 Columns', 'graingrow' ),
        'portfolio-five-columns'     => esc_html__( '5 Columns', 'graingrow' )
...
function themesflat_sanitize_layout_product( $input ) {
    $valid = array(        
        'fullwidth'         => esc_html__( 'No Sidebar', 'graingrow' ),
        'sidebar-right'           => esc_html__( 'Sidebar Right', 'graingrow' ),
        'sidebar-left'         => esc_html__( 'Sidebar Left', 'graingrow' )
    );
    if ( array_key_exists( $input, $valid ) ) {
