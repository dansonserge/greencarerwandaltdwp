...
function themesflat_posted_category( $layout = '' ) { 	
	if ( has_category() ) {
		echo '<div class="post-categories">'.esc_html__("In - ",'graingrow');
			the_category( ', ' );
		echo '</div>';
	}	
}
...
function themesflat_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $tags_list && is_single() ) {
			$tags_links = sprintf( '<div class="tags-links"><h5>' . esc_html__( 'Popular Tag :', 'graingrow' ) . '</h5>' . esc_html__( ' %1$s', 'graingrow' ) . '</div>', $tags_list  );
...
function themesflat_post_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'graingrow' ); ?></h2>
		<ul class="nav-links clearfix">
			...
			if ( is_attachment() ) :
				...
				$prev = esc_html__( 'Published In', 'graingrow' );
				$date = get_the_date();
				...
				if( is_object( $prevPost ) ){
					$prevthumbnail = get_the_post_thumbnail($prevPost->ID);
					$prev_title = get_the_title($prevPost->ID);
					$prev = esc_html__( 'Previews', 'graingrow' );
					$date = get_the_date();
				...
				if( is_object( $nextPost ) ){
					...
					$next = esc_html__( 'Next', 'graingrow' );
					$date = get_the_date();
