...
$links = paginate_links( array(
	...
	'prev_text' => ( '<i class="icon-graingrow-angle-left"></i>' ),
	'next_text' => ( '<i class="icon-graingrow-angle-right"></i>' ),
) );
...
$next_link = get_next_posts_link( esc_html__( 'Next', 'graingrow' ) );
$prev_link = get_previous_posts_link( esc_html__( 'Previous', 'graingrow' ) );
if (themesflat_get_opt('text_loadmore') != '') {
	$text = sprintf('%1$s',themesflat_get_opt('text_loadmore'));
}
$loadmore_text = apply_filters('themesflat/template/loadmore_text',$text);
